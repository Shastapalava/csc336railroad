...
BEGIN
        ...
        /* check this status variable in your prog 0 means no good 1 means good 2 means no selection made*/
                ...
                if trip_direction=0
                  ...
                  set trip_still_avail=(select min(sf_seats_free) from Seats_Free where sf_seg_id between seg0 and seg1);

                if trip_still_avail <=0

                        then    truncate table avail_trains;
                ...
                /* get passenger id from name or create passenger */
                        set passenger_exists=(select count(passenger_id) from Passengers where passenger_lname=pass_lname and passenger_fname=pass_fname);
                        if passenger_exists > 0
                         then set this_passenger=(select passenger_id from Passengers where passenger_lname=pass_lname and passenger_fname=pass_fname);
                        else
                        insert into Passengers (passenger_lname, passenger_fname, passenger_mop) values(pass_lname, pass_fname, pay_method);
                        set this_passenger=(select  passenger_id from Passengers  where passenger_lname=pass_lname and passenger_fname=pass_fname);
                        end if;
                                                                               /* calculate fare-- you may want to do this in get_avail_trains as well*/
                                                                                                              set this_trip_fare=(select sum(Seg_fare) from Segments where segment_id between seg0 and seg1);
select this_trip_fare;                                                         /*display variables*/

select "variables";
select this_start_station, this_end_station, this_train, this_trip_date, this_passenger, this_trip_fare, seg0, seg1;
/* now write ticket*/
                                                                                                                     insert into Tickets (trip_starts, trip_ends, trip_train, trip_date, passenger_id,fare) values (this_start_station, this_end_station, this_train, this_trip_date, this_passenger,this_trip_fare);


                                /*finally decrement seats_free*/
                update Seats_Free set sf_seats_free=sf_seats_free-1 where sf_seg_id between seg0 and seg1 and sf_train_id=this_train and sf_date=this_trip_date;
